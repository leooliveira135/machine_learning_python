...
# Implementaçao Perceptron

class Perceptron:
    def __init__(self, amostras, saidas, tx_aprendizado=0.1, epocas=1000, limiar=-1):
    ...
    def treinar(self):
        ...
        while True:
            ...
            for i in range(self.n_amostras):
                ...
                # verifica se a saida da rede e diferente da saida desejada
                if y != self.saidas[i]:
                    erro_aux = self.saidas[i] - y
                    for j in range(self.n_atributos + 1):
                        self.pesos[j] = self.pesos[j] + self.tx_aprendizado * erro_aux * self.amostras[i][j]
                    erro = True
